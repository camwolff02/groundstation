...
parser.add_argument("--preamble_length", default=8)
...
async def run_telemetry_loop(
    ...
    telemetry_channel_id: ChannelId,
    camera_channel_id: ChannelId,
    cap: cv2.VideoCapture | None,
    mcap_writer: Writer | None = None,
) -> None:
    while True:
        now = time.time_ns()
    ...
    async with FoxgloveServer(args.address, args.port, args.server_name) as server:
        ...
        if args.enable_logging:
            ...
            log_path = os.path.join(
                args.log_dir, f"{args.rocket_name}_{timestamp}.mcap"
            )

            with open(log_path, "wb") as f, Writer(f) as mcap_writer:
